    ...
    public static partial class SpringBoneSerialization
    {
        ...
        )
        {
            ...
            if (actualImportSettings.ImportCollision)
            {
                ...
                {
                    Debug.LogError("Dynamics setup failed: There is an error in the original data");
                    return false;
                }
            }
            ...
            if (hasErrors)
            {
                var errorMessage = "Some errors appear in the dynamics setup \n"
                    + "Do you want to make only normal ones?";
        ...
        private static bool VerifyVersionAndDetectContents(string recordText, ImportSettings importSettings)
        {
            ...
            {
                Debug.LogError("SpringBoneSetup: Could not read the original text data！\n\n" + exception.ToString());
                return false;
            }
            ...
            else
            {
                ...
                {
                    Debug.LogError("SpringBoneSetup: Data version not supported！\nVersion: " + version.ToString());
                    return false;
                }
...
#pragma warning restore 0649
        ...
        )
        {
            ...
            foreach (var sourceRecord in sourceRecords)
            {
                ...
                if (sourceRecord.name.Length == 0)
                {
                    ...
                    Debug.LogError("There is a base object with no name specified");
                    isRecordValid = false;
                }
                ...
                {
                    Debug.LogError(sourceRecord.name + " : Parent name not specified");
                    isRecordValid = false;
                }
                else if (!validParentNames.Contains(parentName))
                {
                    Debug.LogError(sourceRecord.name + " : Parent not found: " + parentName);
                    isRecordValid = false;
                }
        ...
        )
        {
            ...
            foreach (var sourceRecord in sourceRecords)
            {
                ...
                {
                    Debug.LogError(baseData.boneName + " : Object not found");
                    isRecordValid = false;
                }
                ...
                {
                    Debug.LogError(baseData.boneName + " : Base point name is not specified");
                    isRecordValid = false;
                }
                else if (!validPivotNames.Contains(pivotName))
                {
                    Debug.LogError(baseData.boneName + " : Base object not found: " + pivotName);
                    isRecordValid = false;
                }
                ...
                if (missingColliders.Any())
                {
                    ...
                    Debug.LogWarning(
                        baseData.boneName + " : No collider found:\n"
        ...
        )
        {
            ...
            {
                Debug.LogError("Bone not found: " + baseData.boneName);
                return false;
            }
            ...
            if (pivotNodeName.Length > 0)
            {
                ...
                {
                    Debug.LogError("Pivot object not found: " + pivotNodeName);
                    pivotNode = null;
                }
            }
            if (pivotNode == null)
        ...
        private class ParsedSpringBoneSetup
        {
            ...
            )
            {
                ...
                {
                    Debug.LogWarning("SpringBoneSetup: some colliders not found");
                }
            ...
            )
            {
                ...
                foreach (var skippedRecord in skippedBoneRecords)
                {
                    Debug.LogWarning(skippedRecord.baseData.boneName
                        + "\nNot created because it is not in the bone list");
                }
