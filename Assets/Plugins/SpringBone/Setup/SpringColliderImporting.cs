    ...
    public static partial class SpringColliderSerialization
    {
        public class ParsedColliderSetup
        {
            ...
            public static ParsedColliderSetup ReadColliderSetupFromText(GameObject colliderRoot, string recordText)
            {
                ...
                {
                    Debug.LogError("SpringColliderSetup: Could not read the original text data！\n\n" + exception.ToString());
                    return null;
                }
        ...
        private static Transform CreateNewGameObject(Transform parent, string name)
        {
            ...
            if (newObject == null)
            {
                Debug.LogError("Could not create new object: " + name);
                return null;
            }
...
#pragma warning restore 0649
        ...
        )
        {
            ...
            {
                Debug.LogError("Parent not found: " + serializer.parentName);
                return null;
            }
        ...
        )
        {
            ...
            if (objectName.Length == 0)
            {
                ...
                Debug.LogError("There are cases where the collider name is not specified");
                isRecordValid = false;
            }
            ...
            {
                Debug.LogError(objectName + " : Parent name not specified");
                isRecordValid = false;
            }
            else if (!validParentNames.Contains(parentName))
            {
                Debug.LogError(objectName + " : Parent not found: " + parentName);
                isRecordValid = false;
            }
        ...
        )
        {
                ...
                .Where(sourceRecord => VerifyTransformRecord(sourceRecord, validParentNames)))
            {
                ...
                {
                    Debug.LogError(sourceRecord.name + " : Name is duplicated");
                    isValidRecord = false;
                }
        ...
        )
        {
            ...
            foreach (var sourceRecord in colliderRecords)
            {
                ...
                {
                    Debug.LogError(objectName + " : linkedRenderer not found: " + linkedRendererName);
                    isValidRecord = false;
                }

                if (newValidRecords.Any(item => item.GetBaseInfo().transform.name == objectName))
                {
                    Debug.LogError(objectName + " : Name is duplicated");
                    isValidRecord = false;
                }
